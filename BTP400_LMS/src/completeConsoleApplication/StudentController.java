...
public class StudentController {
	...
	public void run() {
		...
		if (loggedIn) {
			System.out.println(
					"Enter '1' to search a book by its name. "
					+ "Enter '2' to view a report of all books. "
					+ "Enter '3' to Request an issue by name. "
					+ "Enter '4' to view a list of borrowed books. "
					+ "Enter 'X' to exit.");
		...
		while (!selection.equalsIgnoreCase("X")) {
			try {
				...
				case "3": {
					System.out.println(
							"Request an issue: Please enter the name of the book you would like to be issued.");
			...
			if (requestedBook.isIssued()) {
				...
				for (IssuedBook issue : issued) {
					if (issue.getBookID() == id && issue.getStudentID() == this.student.getStudentID()) {
						...
						controller.log(new Date() + ": Student #" + this.student.getStudentID()
								+ " issue request unsuccessful.");
						return;
					}
				}
				...
				for (WaitTicket ticket : tickets) {
					...
					if (ticket.getItemID() == requestedBook.getBookID()
							&& ticket.getStudentID() == this.student.getStudentID()) {
				...
				controller.addWaitTicket(new WaitTicket(controller.getFreeIndex(Database.getWaitTicketPath()),
						this.student.getStudentID(), new Date().toString(), requestedBook.getBookID()));
				controller
						.log(new Date() + ": Student #" + this.student.getStudentID() + " has been given a WaitTicket");

			} else {

				controller.addIssuedBook(new IssuedBook(controller.getFreeIndex(Database.getIssuedBookPath()),
						requestedBook.getBookID(), this.student.getStudentID(), new Date().toString()));
				...
				if (controller.issueBook(id)) {
					...
					controller.log(new Date() + ": Student #" + this.student.getStudentID()
							+ " has requested to be issued Book: " + requestedBook.getBookName() + "("
							+ requestedBook.getBookID() + ")");
				} else {
					System.out.println("Book could not be issued.");
					controller.log(
							new Date() + ": Student #" + this.student.getStudentID() + " issue request unsuccessful.");
	...
	public void requestIssue(String bookName) {
		...
		if (requestedBook.isIssued()) {
			...
			for (IssuedBook issue : issued) {
				if (issue.getBookID() == requestedBook.getBookID()
						&& issue.getStudentID() == this.student.getStudentID()) {
			...
			if (controller.issueBook(requestedBook.getBookID())) {
				...
				System.out.println("Book is now issued");
				controller.log(
						new Date() + ": Student #" + this.student.getStudentID() + " has requested to be issued Book: "
								+ requestedBook.getBookName() + "(" + requestedBook.getBookID() + ")");
			} else {
				System.out.println("Book could not be issued.");
				controller
						.log(new Date() + ": Student #" + this.student.getStudentID() + " issue request unsuccessful.");
